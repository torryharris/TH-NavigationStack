...
@synthesize allCountryArr,filteredCountryArr,clickedCountryName;
...
- (void)viewDidLoad
{
    ...
    searchBarReagion.placeholder = @"Search";
                ...
                for (UIView* view in [[sender superview] subviews]) {
                    if([view isKindOfClass:[UILabel class]])
                    {
                        ...
                        if (label.tag == 1) {
                            ...
                            label.frame = CGRectMake(80, 5, 155, 35);
...
-(void)loadHeadingData:(NSString *)headingName withSubHeadings:(NSMutableArray *)subHeadingArray withFrame:(CGRect)clickedFrame{
    ...
    [UIView animateWithDuration:0.5 delay:0 options:UIViewAnimationOptionBeginFromCurrentState|UIViewAnimationOptionTransitionCrossDissolve|UIViewAnimationOptionCurveEaseOut|UIViewAnimationOptionAllowAnimatedContent animations:^{
        ...
        self.state = STATE_DETAILS_MENU;
        self.title = @"Detailed Menu";
        searchBarReagion.placeholder = [NSString stringWithFormat:@"Search in %@ ",headingName];
    }];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    customCell.countryImg.image = [UIImage imageNamed:[NSString stringWithFormat:@"%@",[countryDictinary objectForKey:@"subheadingImg"]]];
